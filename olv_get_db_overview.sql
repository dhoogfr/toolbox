/* run this script as a dba user, passing the logfile as parameter
   This script has been created to assist in the migrations of the 9i databases at OLV
       ...
       @olv_get_db_overview.sql <logfile>
...
from dba_objects
where owner not in ('DBSNMP','ORACLE_OCM','OUTLN','PUBLIC','SYS','SYSMAN','SYSTEM', 'TSMSYS','WMSYS')
...
where grantee not in 
        ( 'SYS', 'SYSTEM', 'TSMSYS', 'WMSYS','SYSMAN', 'OUTLN', 'DBSNMP', 'PERFSTAT', 'UPTIME',
          ...
          'MGMT_USER','MGMT_VIEW','OEM_ADVISOR','ORACLE_OCM','SCHEDULER_ADMIN','DIP'
         )
...
column run_count format 9999 heading RC
column job_name format a28
column next_run_date format a35
column last_start_date format a35

select owner, job_name, state, run_count, failure_count, 
...
select owner, job_name, -- to_char(log_date, 'DD/MM/YYYY HH24:MI:SS') log_date, 
       ...
       run_duration, status
from ( select owner, job_name, log_date, req_start_date, actual_start_date, 
              run_duration, status,
              row_number () over
...
where rn <= 10
order by owner, job_name, jrd.log_date desc;
...
select to_char(actual_start_date, 'DD/MM/YYYY HH24:MI:SS') start_date,
       owner, job_name, run_duration, status
from ( select actual_start_date, owner, job_name, run_duration, status
       from dba_scheduler_job_run_details

...
prompt err: unxpstealcnt / ssolderrcnt / nospaceerrcnt
...
from ( select undotsn,
              ...
              sum(undoblks)
                over 
                    ( order by end_time 
                      range ( tuned_undoretention/24/60/60 
--                              from v$parameter 
--                              where name = 'undo_retention'
                            ) preceding
                    )
                * ( select block_size 
                  ...
                  maxqueryid, unxpstealcnt || '/' || ssolderrcnt  || '/' || nospaceerrcnt err
       from v$undostat 
) U
